  ...
  @section('content')
    <div class="page-header">
        ...
        <div class="form-group" id="filter">
            <div class="row">
                <div class="col-md-3" id="filterProvince">
                    <label class="control-label">Provinsi</label>
                    <select class="form-control" id="selectProvince">
                        <option>Loading Data..</option>
                    </select>
                </div>
                <div class="col-md-3" id="filterCity">
                    <label class="control-label">Kota/Kabupaten</label>
                    <select class="form-control" id="selectCity">
                        <option>Loading Data..</option>
                    </select>
                </div>
                <div class="col-md-3" id="filterDistrict">
    ...
    <div class="panel">
            ...
            <div class="panel-body">
                <table class="table table-striped">
                    <caption>Berdasarkan Rukun Warga</caption>
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>RW</th>
    ...
    <script type="text/javascript">
        ...
        whereClause(penduduk,clauseArray).get().then((querySnapshot)=>{
             ...
             querySnapshot.forEach((doc)=>{
                 if(doc && doc.exists){
                    var data = doc.data();
                    var exist = dataExists(data.rw,dataArray);
                    if(!exist){
                        addData(data.rw, dataArray);
                    }
                }
             });
            ...
            dataArray.forEach((data)=>{
                ...
                dataContainer.append(
                    '<tr>'+
                        ...
                        '<td><a href="/rw/'+province+'/'+city+'/'+district+'/'+subdistrict+'/'+data.name+'">'+data.name+'</a></td>'+
                        '<td>'+data.count+'</td>'+
                    '</tr>'
                );
                counter++;
             ...
             $('#pageTitle').html('<strong>Cakupan Wilayah : </strong> '+province+' > '+city+' > '+district+' > '+subdistrict);
        });
        ...
        selectSubdistrict.on('change',function(){
            ...
            whereClause(penduduk,arrayClause).get().then((querySnapshot)=>{
                ...
                dataArray.forEach((data)=>{
                    ...
                    dataContainer.append(
                        '<tr>'+
                            '<th  scope="row">'+counter+'</th>'+
                            '<td><a href="/rw/'+selectedProvince+'/'+selectedCity+'/'+selectedDistrict+'/'+selectedSubdistrict+'/'+data.name+'">'+data.name+'</a></td>'+
                            '<td>'+data.count+'</td>'+
                        '</tr>'
                    );
                    counter++;
            ...
            $('#pageTitle').html('<strong>Cakupan Wilayah : </strong> '+selectedProvince+' > '+selectedCity+' > '+selectedDistrict+' > '+selectedSubdistrict);
