    ...
    <div class="panel">
        ...
        <div class="panel-body">
            <table class="table table-striped">
                <caption>Berdasarkan Desa/Kelurahan</caption>
                <thead>
                    <tr>
                        <th>#</th>
    ...
    <script type="text/javascript">
        ...
        whereClause(penduduk,clauseArray).get().then((querySnapshot)=>{
             ...
             querySnapshot.forEach((doc)=>{
                 if(doc && doc.exists){
                    var data = doc.data();
                    var exist = dataExists(data.kelurahan
                        ,dataArray);
                    if(!exist){
                        addData(data.kelurahan, dataArray);
                    }
                }
             });
            ...
            dataArray.forEach((data)=>{
                ...
                dataContainer.append(
                    '<tr>'+
                        ...
                        '<td><a href="/kelurahan/'+province+'/'+city+'/'+district+'/'+data.name+'">'+data.name+'</a></td>'+
                        '<td>'+data.count+'</td>'+
                    '</tr>'
                );
                counter++;
             ...
             $('#pageTitle').html('<strong>Cakupan Wilayah : </strong> '+province+' > '+city+' > '+district);
        });
        ...
        selectDistrict.on('change',function(){
            ...
            whereClause(penduduk,arrayClause).get().then((querySnapshot)=>{
                 ...
                 querySnapshot.forEach((doc)=>{
                     if(doc && doc.exists){
                        var data = doc.data();
                        var exist = dataExists(data.kelurahan,dataArray);
                        if(!exist){
                            addData(data.kelurahan,dataArray);
                        }
                    }
                ...
                dataArray.forEach((data)=>{
                    ...
                    dataContainer.append(
                        '<tr>'+
                            '<th  scope="row">'+counter+'</th>'+
                            '<td><a href="/kelurahan/'+selectedProvince+'/'+selectedCity+'/'+selectedDistrict+'/'+data.name+'">'+data.name+'</a></td>'+
                            '<td>'+data.count+'</td>'+
                        '</tr>'
                    );
                    counter++;
            ...
            $('#pageTitle').html('<strong>Cakupan Wilayah : </strong> '+selectedProvince+' > '+selectedCity+' > '+selectedDistrict);
